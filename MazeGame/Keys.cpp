...
// generates the coordinates of the keys
void generateKeys() {
	playerKeys = 0;
	currentKeysCount = 0;
...
// draws the keys on the coordinates generated in the previous function
void drawKeys() {
...
// erases the key from the console
void eraseKey(int index) {
	gotoxy(1 + key[index][1] * 5, 2 + key[index][0] * 2); cout << "    ";
}

// checks if the player is at the same coordinates as a key
...
// adds 1 to the sum of the keys the player got and erases the key from the console
void takeKey() {
	playerKeys++;
...
// subtracts 1 from the sum of the keys the player got 
void useKey() {
	playerKeys--;
}

// returns the sum of the keys the player got
int getPlayerKeys() {
	return playerKeys;
}
